...
[bloodflow, cvector] = heart(bloodflow, cvector, mass);
...
[BFbrainj, cvectorbrainj] = brain(BFbraini, cvector, mass);
[BFliverj, cvectorliverj] = liver(BFliveri,cvector,gender,mass,LW);
[BFotherbloodj, cvectorotherbloodj] = otherblood(BFotherbloodi, cvector, carbs, calciumintake, sodiumintake, ironintake); 
...
[BFkidneyj, cvectorkidneyj] = kidney(BFkidneyi, cvectorotherbloodj,RQ);
...
Cout(3) = Cin(3);
Cout(4) = Cin(4);
Cout(5) = (flow*Cin(5) + (.8*carbs)/180.156)/flow; %130g of carbs, 80 percent is directly translated to glucose, 20% is fructose and galactose which goes to the liver and may be converted to glucose at a later stage so another term is incoming
