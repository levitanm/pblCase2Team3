...
[BFotherbloodj, cvectorotherbloodj] = otherblood(BFotherbloodi, cvector1, carbs, calciumintake, sodiumintake, ironintake, Ci, RQ); 
...
[BFliverj, cvectorliverj] = liver(BFliveri,cvectorliverin,gender,mass,LW, Ci);
...
figure %plot of all component concentrations on same graph
hold on
...
plot(0:loop(end),cCatrack)
plot(0:loop(end),cIrontrack)
title('Component Concentrations Over Time')
xlabel('Time in Minutes')
ylabel('Concentration') %in mol/mL (except for Erythrocytes which is %Volmume)
legend('Erythrocyte Concentration %Volume','O2 Concentration in mol/mL','CO2 Concentration in mol/mL','HCO3 Concentration in mol/mL','Glucose Concentration in mol/mL','Na Concentration in mol/mL','Ca Concentration in mol/mL','Iron Concentration in mol/mL')
hold off
end
...
% breatingrate = 15; %Bert told me this was the average number of breaths per minute
% alveoliO2volume = x*breatingrate; %x is the amount of oxygen added to the alveoli in 1 minute 
% alveolipartialpressure = (moles*.08206*300.15)/alveoliO2volume; %moles is # of moles of O2 in alveoli
% atmospherepartialpressure = .2;
% O2tobloodstreampercent = 5/alveoliO2volume; %using 5 mL as standard value to get percentage
% if alveolipartialpressure < atmospherepartialpressure
%     additionalpressure = atmospherepartialpressure - alevolipartialpressure;
%     additionalbreatingrate = (moles*.08206*300.15)/(additionalpressure*x);
%     breatingrate = breatingrate + additionalbreatingrate;
% end
% newalveoliO2volume = x*breatingrate;
% O2toblood = (newalveoliO2volume*O2tobloodstreampercent)/density; %conversion from mL/min to mol/min
% Cout(2) = (Cvector(2)*vblood + O2toblood - nO2cons);
% inputpartialpressure = (.08206*300.15*Cvector(2)*1000); %Cvector(2) is in mol/mL so multiplying by 1000 will be mol/L and thats enough to get pressure
    Cout = [];
...
%                                          %hemoglobin, mol/mL
    %nO2cons = CO2bloodmol*vblood*1000; %molar flow rate of oxygen consumed, mol/min - can we somehow...
                                       ...
                                       %equation?
    nglucosecons = nO2cons/6; %molar flow rate of glucose consumed, mol/min
...
MO2cons = 0.184*Ci;
MGlucosecons = .00003108*(brainmass/100); %discrepancy between amount of oxygen consumed and amount of glucose consumed based on the cellular respiration equation 
...
MGlucosej = Mvector(5)-(.00003108*(brainmass/100)); %5.6 mg/min = .00003108 mol/min
...
function [bloodflowj, cvectorout]=liver(V,cvectori,G,mass,LW, Ci)
...
function [outflow, Cout] = otherblood(flow, Cin, carbs, calciumintake, sodiumintake, ironintake, Ci, RQ)
Cout = [];
Cout(1) = Cin(1);
