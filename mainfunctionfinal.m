function mainfunctionfinal()

age = 50;%30; %Age in years
gender = 0; %Gender, 0=male, 1=female
mass = 70; %Weight in kg
anemia = 1; %Is the patient anemic? 1 if yes, 0 if no
bleed = 521400000; %Bleeding rate in erythrocytes/min, if anemic
FerritinStores = 0; %Stored moles of iron in liver as ferritin. 0.0179 is standard, but can be adjusted to represent normal levels, severe anemia, or intervention

RQ=0.825;%reaction quotient for the body tissues generally

%if/else statement that sets heart rate based on age, in bpm
if (40>=age) 
    baseheartrate = 60;
else 
    baseheartrate= 70;
...
%Intake of various nutrients included in the model. Can be adjusted to
%represent the patient's diet
carbs=130; %carbs in grams
calciumintake=1000; % calcium intake in milligrams
sodiumintake=500; % sodium intake in milligrams

%if statement that varies iron intake between genders, in order to account
%for differences in diet as well as menstruation in females
if gender==1
    ironintake=8;
else
    ironintake=18;
...
bloodflow0 = 1000*basebloodweight/1.06; %blood flow out of heart per minute in mL, divided by density of 1.06 g/mL 
...
basehemoglobin=34.52243959*basebloodweight*10*cE0; %total hemoglobin, in grams, that is initially  present in the body
hemoglobin=basehemoglobin;
valueofhemoglobin=hemoglobin;
...
%initialize tracking vectors for all components, for graphing purposes
cEtrack=[cvector0(1)];
cO2track=[cvector0(2)];
...
hemovec=[valueofhemoglobin];

%base blood pressure and associated vector, begins at an average of 100
%mmHg
baseBP=100;
BPvec=[baseBP];
...
for loop=1:1440
    ...
    %maintain nutrient supply to the body
    ...
    [BFotherbloodj, cvectorotherbloodj] = otherblood(BFotherbloodi, cvector1, carbs, calciumintake, sodiumintake, ironintake, Ci, RQ, anemia, bleed, basehemoglobin, hemoglobin);
    ...
    %Weighted average to get a new concentration vector
    ...
    %A is a matrix of values across the system, for diagnostic purposes
    A = [cvector; cvector1; cvectorbrainj; cvectorotherbloodj; cvectorliverj; cvectorkidneyj; cvector0];
    %equation that calculates blood pressure based on viscosity
    BP=baseBP*(1.8886*(cvector0(1)/cE0)-0.8886);
    BPvec=[BPvec BP];
...
xlabel('Time in Minutes')
ylabel('Hemoglobin in g')

figure
plot(0:loop(end),BPvec,'k','LineWidth',2, 'DisplayName', 'Iron Concentration')
...
Co = 0.21e-4; %concentration of oxygen in alveoli (21% b/c air)
...
nO2cons = 0.05*Ci/(hemoglobin/basehemoglobin);
...
nCO2cons = nCO2i*(1-(48/52)); %vblood(mL/min) and concentration of blood consumed (mol/mL)
...
%cvectorout=[cE,cO2,cCO2,cHCO3,cGlucose,cNa,cCa,cFe)
...
%Consume 0.8% of RBCs based on body size
RBCcons=5.55555e-6*cvectori(1)*Vblood;
%Find pure volume of RBCs now present based on number and density
...
%Calculates the concentration flow rate of Na+ out
%Assume no generation of Na+, only in, out, and consumption
...
%Assume no generation of Ca2+

%Equation for consumption of glucose by the liver
...
%If statement extracts excess iron from blood and stores it as ferritin,
%supplies iron-deficient blood with iron until ferritin stores are depleted
if Mirondif < 0
...
%Erythrocytes and iron are conserved in the kidney
MEj=Mvector(1);
MIronj=Mvector(8);
...
PNaconc=cvectori(6)*1e6; %plasma sodium concentration, converted to mmol/L for computations
...
%Equations which calculate consumption of calcium
...
%Calculate consumption of glucose
...
MHCO3j=Mvector(4)+MHCO3gen-MHCO3cons;
...
function [outflow, Cout] = otherblood(flow, Cin, carbs, calciumintake, sodiumintake, ironintake, Ci, RQ, anemia, bleed, basehemoglobin, hemoglobin)
Cout = [];
outflow=flow;
...
if anemia == 0
    ...
    vRBCin = flow*Cin(1);
    %Number of Red blood cells lost per minute, can be adjusted based on
    %intensity of bleeding at top of code
    RBClost = bleed;
...
Cout(5) = (flow*Cin(5) + (.8*carbs)/(180.156*1440))/flow; %130g of carbs, 80 percent is directly translated to glucose, 20% is fructose and galactose which goes to the liver and may be converted to glucose at a later stage
